...
interface InvoiceModalProps {
  ...
  shippingRates: Record<string, number>; // New prop
}
...
const InvoiceModal: React.FC<InvoiceModalProps> = ({ isOpen, editingInvoice, onClose, onSave, shippingRates }) => {
    ...
    if (isOpen) {
      if (editingInvoice) {
        ...
        setFormData({
          ...
          quantity: 1,
          price: 0,
          shippingCost: 0, // Will be updated by governorate selection if not editing
          watermarkText: '',
          websiteUrl: '',
        });
      }
  ...
  useEffect(() => {
    const generateQR = async () => {
      const urlToEncode = formData.websiteUrl || window.location.href;
      try {
        const dataUrl = await QRCode.toDataURL(urlToEncode, { width: 120, margin: 1 });
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setFormData(prev => {
      ...
      // Auto-update shipping cost if governorate changes
      if (name === 'governorate') {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm overflow-y-auto">
      <div className="bg-white w-full max-w-5xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[95vh] animate-in zoom-in-95 duration-200">
        ...
        <div className="flex flex-col md:flex-row h-full overflow-hidden">
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-1 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-4 focus:ring-indigo-500/10 outline-none font-bold"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  <button onClick={() => fileInputRef.current?.click()} className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-white border-2 border-dashed border-gray-300 rounded-xl hover:border-indigo-500 hover:text-indigo-600 transition-all text-gray-500 font-bold">
                    <ImageIcon size={20} />
                    <span>رفع صورة</span>
                  </button>
              ...
              <div>
                ...
                <input type="text" name="watermarkText" value={formData.watermarkText} onChange={handleChange} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-4 focus:ring-indigo-500/10 outline-none mb-3" />
                <label className="block text-sm font-bold text-gray-700 mb-2">رابط الموقع (QR)</label>
                <input type="text" name="websiteUrl" value={formData.websiteUrl} onChange={handleChange} className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-4 focus:ring-indigo-500/10 outline-none text-left" dir="ltr" />
              </div>
              ...
              <div className="border-t pt-4 mt-4">
                 ...
                 <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        <input type="text" name="phone1" value={formData.phone1} onChange={handleChange} placeholder="رقم الهاتف" className="w-full px-4 py-3 bg-white border rounded-xl" />
                        <input type="text" name="phone2" value={formData.phone2} onChange={handleChange} placeholder="رقم آخر" className="w-full px-4 py-3 bg-white border rounded-xl" />
                    </div>
                    ...
                    <div>
                      ...
                      <select name="governorate" value={formData.governorate} onChange={handleChange} className="w-full px-4 py-3 bg-white border rounded-xl font-bold text-indigo-900">
                    ...
                    <div>
                       ...
                       <input type="number" name="shippingCost" value={formData.shippingCost} onChange={handleChange} placeholder="توصيل" className="w-full px-4 py-3 bg-white border rounded-xl font-bold text-green-700" />
                    </div>
                 </div>
              </div>
            </div>
          ...
          <div className="flex-1 p-10 overflow-y-auto relative bg-white flex flex-col justify-between" id="invoice-printable">
            ...
            <div className="relative z-10 space-y-8 flex-1">
              ...
              <div className="flex justify-between items-start border-b-4 border-indigo-600 pb-6">
                <div className="flex flex-col gap-2">
                  ...
                  <h2 className="text-xl font-bold text-indigo-600">{formData.pageTitle}</h2>
            ...
            <div className="relative pt-12 mt-8 flex justify-between items-end">
               ...
               <div className="flex items-center gap-3 bg-white p-2 border border-gray-100 rounded-lg shadow-sm">
                 <div className="text-right">
                   <p className="text-[10px] text-gray-400 font-bold mb-1">امسح الكود</p>
                   <p className="text-[10px] text-indigo-600 font-bold max-w-[120px] truncate" dir="ltr">
                     {formData.websiteUrl || 'Mr & Mrs Fashion'}
                   </p>
                 </div>
                 {qrCodeImg && (
                   <img src={qrCodeImg} alt="QR Code" className="w-16 h-16 object-contain" />
                 )}
               </div>
            </div>
